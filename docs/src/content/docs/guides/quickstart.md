...
Open `https://dash.example.com`, using the hostname you chose. The Kubernetes Dashboard should load through Cloudflare Tunnel. Your hostname is now public, so to require authentication in front of it, continue with [Protect a hostname with Cloudflare Access](/how-to/protect-with-cloudflare-access/).
