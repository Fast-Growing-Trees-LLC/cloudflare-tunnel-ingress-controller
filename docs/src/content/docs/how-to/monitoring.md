...
In steady state `operation="update"` does not move. A reconcile that finds the application already matching what the Ingress asks for plans no write, so a counter that keeps climbing while nothing changed means something outside the controller is editing those applications, most often a hand edit in the Zero Trust dashboard being reverted on every resync.
