...
// accessApplicationsPerPage is the page size used when listing Access
// applications. Bigger pages mean fewer round trips; the value is otherwise
// arbitrary and the loop is correct for any of them.
const accessApplicationsPerPage = 50

// listAccessApplications returns every Access application in the account.
//
// It walks the pages itself rather than letting cloudflare-go auto-paginate,
// because the SDK's auto-pagination corrupts the results it returns.
// ListAccessApplications declares one AccessApplicationListResponse ahead of
// the page loop and unmarshals every page into it
// (cloudflare-go@v0.117.0/access_application.go, "var r
// AccessApplicationListResponse" above the "for {"). Unmarshalling a JSON array
// into a non empty slice reuses the existing backing array and merges each
// object into the struct already sitting there, so decoding page 2 rewrites the
// page 1 elements in place - after they were appended to the result by value.
// The scalar fields of those copies are safe, they were copied; every slice and
// pointer field still aliases the mutated array.
//
// Observed against a 37 application account: the application at page 1 index 0
// came back carrying the first policy of the application at page 2 index 0 in
// precedence slot 1, its own policy left in slot 2. That is drift the
// controller can never reconcile away - accessAppNeedsUpdate compares policy
// IDs, so every reconcile planned another update of a security object - and
// Tags aliases the same way, which decides whether the controller believes it
// owns an application at all.
//
// Passing an explicit page disables the SDK's auto-pagination, so each call
// decodes into its own response value and the aliasing cannot happen. Same
// class of upstream bug as the FIXME in GetTunnelIdFromTunnelName, but the
// fixed oversized page there would silently truncate this listing, and a
// missing owned application looks like an absent one: duplicate creates and
// un-reaped orphans.
//
// listAccessPolicies is unaffected: it reads only AccessPolicy.ID, a string,
// which the copy owns.
//
// The loop terminates on the local page counter rather than on the page number
// the API echoes back, so a response that keeps reporting page 1 costs one
// wasted request instead of spinning forever. A page that errors fails the
// whole listing: a partial one is indistinguishable from an account whose
// applications are gone, and this listing is what drives the deletions.
func (t *TunnelClient) listAccessApplications(ctx context.Context) ([]cloudflare.AccessApplication, error) {
	var applications []cloudflare.AccessApplication
	for page := 1; ; page++ {
		batch, info, err := t.cfClient.ListAccessApplications(ctx, cloudflare.AccountIdentifier(t.accountId), cloudflare.ListAccessApplicationsParams{
			ResultInfo: cloudflare.ResultInfo{Page: page, PerPage: accessApplicationsPerPage},
		})
		if err != nil {
			return nil, err
		}
		applications = append(applications, batch...)
		// a response without result_info reports no pages at all, and is the
		// only page there is
...
func (t *TunnelClient) planAccessApplications(ctx context.Context, exposures []exposure.Exposure) (accessPlan, error) {
	...
	applications, err := t.listAccessApplications(ctx)
	if err != nil {
