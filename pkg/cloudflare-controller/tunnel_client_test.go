...
// The Access application listing fixtures below reproduce the shape of a real
// account: more applications than fit on one page, the application this
// controller owns first, and a neighbour application whose own policies are
// different. The neighbour is what makes the fixture worth having, see
// Test_sdkAutoPaginationCorruptsAccessApplications.

const WhateverNeighbourPolicy = "8b1c0d7e-4f2a-4b6c-9d31-0a5e7c2f1b42"
const WhateverNeighbourHostname = "neighbour.example.com"
const WhateverAccountId = "account-id"

// neighbourTags are the tags of an application this controller does not own.
// Ownership is decided on tags, so they are part of what a listing must not mix
// up between applications.
var neighbourTags = []string{"someone-elses-tag", "another-tag"}

// accessAppsFixture is the corpus the fixture server pages over: the owned
// application first, filler up to a full page, and the neighbour alone on the
// page after it, so that the neighbour lands on the same index as the owned
// application on a later page.
func accessAppsFixture() []cloudflare.AccessApplication {
	owned := cloudflare.AccessApplication{
		ID:       "app-owned",
	...
	apps := []cloudflare.AccessApplication{owned}
	for len(apps) < accessApplicationsPerPage {
		apps = append(apps, cloudflare.AccessApplication{
			ID:     fmt.Sprintf("app-filler-%d", len(apps)),
			Name:   fmt.Sprintf("filler-%d.example.com", len(apps)),
			Domain: fmt.Sprintf("filler-%d.example.com", len(apps)),
			Type:   cloudflare.SelfHosted,
		})
	}

	return append(apps, cloudflare.AccessApplication{
		ID:       "app-neighbour",
		Name:     WhateverNeighbourHostname,
		Domain:   WhateverNeighbourHostname,
		Type:     cloudflare.SelfHosted,
		Policies: accessPolicies(WhateverNeighbourPolicy),
		Tags:     neighbourTags,
	})
}

// accessAppsFixtureClient serves accessAppsFixture from a real HTTP server and
// returns an SDK client pointed at it. Going through the SDK rather than a fake
// client is the whole point: the defect under test is in the SDK's decoding.
func accessAppsFixtureClient(t *testing.T) *cloudflare.API {
	t.Helper()
	apps := accessAppsFixture()
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if page < 1 || perPage < 1 {
			t.Errorf("access application listing requested page=%q per_page=%q, want both set", query.Get("page"), query.Get("per_page"))
			http.Error(w, "unpaginated request", http.StatusBadRequest)
			return
		}

		start := min((page-1)*perPage, len(apps))
		end := min(start+perPage, len(apps))
		totalPages := (len(apps) + perPage - 1) / perPage
		...
		if err := json.NewEncoder(w).Encode(map[string]any{
			...
			"result_info": map[string]int{
				...
				"count": end - start, "total_count": len(apps), "total_pages": totalPages,
			},
		}); err != nil {
			t.Errorf("encode access application listing: %v", err)
		}
	}))
	t.Cleanup(server.Close)

	api, err := cloudflare.NewWithAPIToken("whatever-token",
		cloudflare.BaseURL(server.URL),
		// the fixture is local, the default 4rps would only add latency
		cloudflare.UsingRateLimit(10000),
	)
	if err != nil {
		t.Fatalf("build fixture client: %v", err)
	}
	return api
}

func findFixtureApp(apps []cloudflare.AccessApplication, id string) (cloudflare.AccessApplication, bool) {
...
// Test_listAccessApplicationsPaginatesWithoutAliasing is the regression test for
// the endless "update access application" loop observed in a sandbox cluster:
// the listing reported a policy the application does not have, so
// accessAppNeedsUpdate saw drift that no update could ever settle.
func Test_listAccessApplicationsPaginatesWithoutAliasing(t *testing.T) {
	client := NewTunnelClient(logr.Discard(), accessAppsFixtureClient(t), WhateverAccountId,
		WhateverTunnelId, WhateverTunnelName, "", exposure.AccessDefaults{Enabled: true})

	applications, err := client.listAccessApplications(t.Context())
	if err != nil {
		t.Fatalf("listAccessApplications() error = %v", err)
	}

	// every page, or an owned application missing from the tail of the listing
	// looks absent: duplicate create, orphan never reaped
	if len(applications) != len(accessAppsFixture()) {
		t.Errorf("listAccessApplications() returned %d application(s), want %d", len(applications), len(accessAppsFixture()))
	}

	owned, found := findFixtureApp(applications, "app-owned")
	if !found {
		t.Fatal("listAccessApplications() did not return the owned application")
	}

	if got, want := policyIDs(owned.Policies), []string{WhateverPolicyA, WhateverPolicyB}; !slices.Equal(got, want) {
		t.Errorf("owned application policies = %v, want %v", got, want)
	}
	if got, want := owned.Tags, ownershipTags(WhateverTunnelName); !slices.Equal(got, want) {
		t.Errorf("owned application tags = %v, want %v", got, want)
	}
	if !hasOwnershipTags(owned, WhateverTunnelName, WhateverTunnelId) {
		t.Error("hasOwnershipTags() = false for the application this controller owns")
	}

	// the acceptance criterion the sandbox failed: a steady state reconcile
	// plans no write at all
	...
	if accessAppNeedsUpdate(owned, spec) {
		t.Error("accessAppNeedsUpdate() = true for an application that already matches the desired state")
	}

	neighbour, found := findFixtureApp(applications, "app-neighbour")
	if !found {
		t.Fatal("listAccessApplications() did not return the neighbour application")
	}
	if got, want := policyIDs(neighbour.Policies), []string{WhateverNeighbourPolicy}; !slices.Equal(got, want) {
		t.Errorf("neighbour application policies = %v, want %v", got, want)
	}
	if hasOwnershipTags(neighbour, WhateverTunnelName, WhateverTunnelId) {
		t.Error("hasOwnershipTags() = true for an application this controller does not own")
	}
}

// Test_sdkAutoPaginationCorruptsAccessApplications pins the upstream defect that
// listAccessApplications works around, so that the workaround is not deleted as
// unnecessary ceremony and so that a dependency bump reports when it can go.
//
// ListAccessApplications decodes every page into one response value declared
// above the loop, and decoding a JSON array into a non empty slice reuses the
// backing array and merges into the structs already in it. Page 1 has already
// been appended to the result by value by then, so its slice and pointer fields
// still alias what page 2 overwrites.
//
// If this test fails, cloudflare-go has fixed it: drop
// the page loop in listAccessApplications and pass a zero
// ListAccessApplicationsParams again.
func Test_sdkAutoPaginationCorruptsAccessApplications(t *testing.T) {
	api := accessAppsFixtureClient(t)

	applications, _, err := api.ListAccessApplications(t.Context(),
		cloudflare.AccountIdentifier(WhateverAccountId), cloudflare.ListAccessApplicationsParams{})
	if err != nil {
		t.Fatalf("ListAccessApplications() error = %v", err)
	}
	...
	// the neighbour's policy landed in the owned application's first precedence
	// slot, its own policy survives in the second: exactly the mixed shape the
	// sandbox reported
	if got, want := policyIDs(owned.Policies), []string{WhateverNeighbourPolicy, WhateverPolicyB}; !slices.Equal(got, want) {
		t.Errorf("cloudflare-go auto-pagination policies = %v, want the corrupted %v; if the SDK is fixed, see this test's comment", got, want)
	}
	// and the same aliasing decides ownership
	if got := owned.Tags; !slices.Equal(got, neighbourTags) {
		t.Errorf("cloudflare-go auto-pagination tags = %v, want the corrupted %v; if the SDK is fixed, see this test's comment", got, neighbourTags)
	}
}
